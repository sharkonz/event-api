...
public class EventIntegrationTest {
    ...
    public void testEventCrudOperations() throws Exception {
        // 1. Create an event.
        Event newEvent = new Event(null, "Test Event", "Description", "Location", LocalDateTime.now(), LocalDateTime.now(), 10, null, null);
        String eventJson = objectMapper.writeValueAsString(newEvent);
        String createdEventResponseBody = mockMvc.perform(post("/events/events")
        ...
        mockMvc.perform(put("/events/events/" + createdEvent.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(updatedEventJson))
                .andExpect(status().isOk());

        // 3. Add another event.
        Event secondEvent = new Event(null, "Second Event", "Description", "Location", LocalDateTime.now(), LocalDateTime.now(), 15, null, null);
        mockMvc.perform(post("/events/events")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(secondEvent)))
                .andExpect(status().isCreated());

        // 4. Retrieve all events and check if both exist.
        mockMvc.perform(get("/events/events/getallevents"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(2))
                .andExpect(jsonPath("$[0].name").value("Updated Event Name"))
                .andExpect(jsonPath("$[1].name").value("Second Event"));

        // 5. Delete one event.
        mockMvc.perform(delete("/events/events/" + createdEvent.getId()))
                .andExpect(status().isNoContent());

        // 6. Retrieve the deleted event and see if it's deleted.
        mockMvc.perform(get("/events/events/" + createdEvent.getId()))
